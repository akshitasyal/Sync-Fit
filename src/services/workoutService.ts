...
import { IExercise, IWorkoutPlan, IWorkoutDay } from "@/types/workout";
...
const adjustStats = (exercise: IExercise, intensity: string, experience: string) => {
...
export const workoutService = {
  generateWeeklyWorkoutPlan: async (userEmail: string, overrideProgram?: string): Promise<IWorkoutPlan> => {
    ...
    if (exercisePool.length < 6) {
      const fallback = await Exercise.find({ program: { $in: ["beginner"] } }).lean() as IExercise[];
      exercisePool = [...exercisePool, ...fallback];
    }

    if (exercisePool.length === 0) {
      exercisePool = await Exercise.find().limit(15).lean() as IExercise[];
    }
    if (exercisePool.length === 0) throw new Error("No exercises found.");
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (split.focus !== "Rest") {
        ...
        selected.forEach(ex => {
          const stats = adjustStats(ex as IExercise, intensity, experience);
          dayExercises.push({
            exerciseId: (ex as any)._id,
            sets: stats.sets,
            reps: stats.reps,
  ...
  getWorkoutPlan: async (userEmail: string): Promise<IWorkoutPlan | null> => {
    await connectToDatabase();
    return await WorkoutPlan.findOne({ userEmail })
      .sort({ createdAt: -1 })
      .populate("days.exercises.exerciseId")
      .lean() as IWorkoutPlan | null;
  },

  getWorkoutPlanById: async (id: string): Promise<IWorkoutPlan | null> => {
    await connectToDatabase();
