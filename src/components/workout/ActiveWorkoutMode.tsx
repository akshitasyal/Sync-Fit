...
export default function ActiveWorkoutMode({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#0b0c10] text-white flex flex-col justify-between overflow-y-auto animate-in fade-in duration-300">
      ...
      <div className="p-3 sm:p-6 border-t border-white/10 flex items-center justify-between gap-2 sm:gap-4 max-w-7xl mx-auto w-full flex-wrap">
        <button
          onClick={prevExercise}
          disabled={currentExIdx === 0}
          className="flex items-center gap-1.5 sm:gap-2 px-3.5 sm:px-5 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl bg-white/10 hover:bg-white/20 disabled:opacity-30 disabled:pointer-events-none text-[10px] sm:text-xs font-black uppercase text-white transition-all cursor-pointer"
        >
        ...
        <button
          ...
          className="px-3.5 sm:px-6 py-2.5 sm:py-3 bg-white/10 hover:bg-white/20 text-gray-300 hover:text-white rounded-xl sm:rounded-2xl text-[10px] sm:text-xs font-black uppercase tracking-wider transition-colors cursor-pointer"
        >
          End &amp; Log
        </button>
