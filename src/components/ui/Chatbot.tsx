  ...
  return (
    <>
      ...
      <button 
        ...
        className={`fixed bottom-6 right-6 p-4 bg-emerald-500 hover:bg-emerald-600 text-white rounded-full shadow-lg shadow-emerald-500/30 transition-all z-50 ${isOpen ? 'scale-0' : 'scale-100 hover:scale-110'}`}
      >
      ...
      <div className={`fixed bottom-6 right-6 w-80 sm:w-96 glass-panel-dark border border-white/10 rounded-2xl shadow-2xl flex flex-col z-50 transition-all duration-300 origin-bottom-right ${isOpen ? 'scale-100 opacity-100' : 'scale-0 opacity-0 pointer-events-none'}`}>
        ...
        <div className="flex-grow p-4 h-80 overflow-y-auto flex flex-col gap-3 scrollbar-thin scrollbar-thumb-emerald-500/20">
          {messages.map((msg, i) => (
            <div key={i} className={`max-w-[85%] rounded-2xl p-3.5 text-sm leading-relaxed shadow-sm ${msg.role === 'user' ? 'bg-emerald-500 text-white self-end rounded-br-none' : 'bg-slate-800 text-slate-200 self-start border border-white/5 rounded-bl-none'}`}>
              {msg.text}
            </div>
          ))}
        </div>
        ...
        <form onSubmit={handleSend} className="p-3 border-t border-white/10 bg-slate-900/50 rounded-b-2xl flex gap-2">
          <input 
            ...
            className="flex-grow bg-slate-950 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-emerald-500/50 transition-colors"
          />
          <button type="submit" disabled={!input.trim()} className="p-2.5 bg-emerald-500 hover:bg-emerald-600 disabled:bg-slate-800 disabled:text-slate-500 text-white rounded-xl transition-colors shrink-0">
