...
// Import verified data from previous checks
// (Copying minimal set here to ensure this script is definitely egg-free)
...
async function totalNuclearWipe() {
  const uri = process.env.MONGODB_URI;
  if (!uri) throw new Error("Missing MONGODB_URI in .env.local");
  await mongoose.connect(uri);

  console.log("💣 INITIALIZING TOTAL NUCLEAR WIPE...");

  // 1. Wipe everything related to tracking
  const sListResult = await ShoppingList.deleteMany({});
  console.log(`🗑️ Deleted ${sListResult.deletedCount} shopping lists.`);

  const planResult = await MealPlan.deleteMany({});
  console.log(`🗑️ Deleted ${planResult.deletedCount} meal plans.`);

  // 2. Wipe ALL meals to ensure no ghost entries or mislabeled categories
  const mealResult = await Meal.deleteMany({});
  console.log(`🗑️ Deleted ${mealResult.deletedCount} total meals from the collection.`);

  // 3. Re-seed verified vegetarian meals
  let inserted = 0;
  for (const meal of vegetarianMeals) {
    try {
      ...
      console.error(`❌ Failed to insert ${meal.name}: ${e.message}`);
    }
  }
  
  console.log(`✅ Re-seeded ${inserted} core vegetarian meals.`);
  console.log("🏁 TOTAL NUCLEAR WIPE COMPLETE.");
  process.exit(0);
}

totalNuclearWipe().catch(console.error);
