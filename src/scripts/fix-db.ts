...
const filesToCopy = [
  ...
  { src: 'nutrition_meal_plan_concept_1775117181330.png', dest: 'features/nutrition.jpg' }
];
...
for (const file of filesToCopy) {
  try {
    const s = path.join(srcDir, file.src);
    const d = path.join(destDir, path.normalize(file.dest));
    fs.copyFileSync(s, d);
